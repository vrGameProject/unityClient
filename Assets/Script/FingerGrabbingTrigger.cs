...
public class HandGrabbingTrigger : MonoBehaviour
{
    ...
    void Start()
    {
        pinch_state_ = PinchState.Released;
        active_object_ = null;
    ...
    protected PinchState GetNewPinchState(HandModel hand_model, Hand leap_hand)
    {
        ...
        if (closest_distance <= trigger_distance)
            return PinchState.Grabbed;
        if (closest_distance <= release_distance && pinch_state_ != PinchState.Released &&
            !ObjectReleaseBreak(current_pinch_position_))
            return PinchState.Releasing;
        return PinchState.Released;
    }

    protected void UpdatePinchPosition(HandModel hand_model)
    {
    ...
    protected void UpdatePalmRotation(HandModel hand_model)
    {
    ...
    void FixedUpdate()
    {
        ...
        UpdatePalmRotation(hand_model);
        UpdatePinchPosition(hand_model);

        if (leap_hand == null)
            return;

        PinchState new_pinch_state = GetNewPinchState(hand_model, leap_hand);
        if (pinch_state_ == PinchState.Grabbed)
        {
            if (new_pinch_state == PinchState.Released)
                OnRelease();
            else if (active_object_ != null)
                ContinueHardPinch();
        }
        else if (pinch_state_ == PinchState.Releasing)
        {
            if (new_pinch_state == PinchState.Released)
                OnRelease();
            else if (new_pinch_state == PinchState.Grabbed)
                StartPinch();
            else if (active_object_ != null)
                ContinueSoftPinch();
        }
        ...
        {
            if (new_pinch_state == PinchState.Grabbed)
                StartPinch();
            else
                Hover();
        }
